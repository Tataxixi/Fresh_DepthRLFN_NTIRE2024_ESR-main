...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        name, data_range = f"{model_id:02}_RLFN_PruneLocal", 1.0
...
def select_dataset(data_dir, mode):
    ...
    if mode == "test":
        ...
        path = [
            (
                p
            ) for p in sorted(glob.glob(os.path.join(data_dir, "DIV2K_LSDIR_test_LR/*.png")))
...
def main(args):
    ...
    if True:
        ...
        # input_fake = torch.rand(3, 256, 256).to(device)
        ...
        with open(json_dir, "w") as f:
            json.dump(results, f)
    if args.include_test:
        fmt = "{:20s}\t{:14s}\t{:14s}\t{:14s}\t{:10s}\t{:10s}\t{:8s}\t{:8s}\t{:8s}\n"
        s = fmt.format("Model", "Val Time [ms]", "Test Time [ms]", "Ave Time [ms]",
                       "Params [M]", "FLOPs [G]", "Acts [M]", "Mem [M]", "Conv")
    else:
        fmt = "{:20s}\t{:14s}\t{:10s}\t{:10s}\t{:8s}\t{:8s}\t{:8s}\n"
        s = fmt.format("Model", "Val Time [ms]", "Params [M]", "FLOPs [G]", "Acts [M]", "Mem [M]", "Conv")
    for k, v in results.items():
        ...
        if args.include_test:
            ...
            s += fmt.format(k, val_time, test_time, ave_time, num_param, flops, acts, mem, conv)
        else:
            s += fmt.format(k,val_time, num_param, flops, acts, mem, conv)
